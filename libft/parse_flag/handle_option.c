...
void display_option_list(FlagContext flag_c)
{
    ...
    for (t_list *tmp = flag_c.opt_lst; tmp; tmp = tmp->next) {
        node = tmp->content;
		ft_printf_fd(1, "----------------------------------------------------------------------------------------------------------------------------------------------------------------------");
        ft_printf_fd(1, CYAN"\nFlag: [-%c] "PURPLE"Power [%d] "YELLOW" Name |%s|"RESET""ORANGE" Type [%s],"GREEN" Min :[%d] "RED"Max [%d] "ORANGE" mult_val: [%s] "BLUE"nb: [%d] "PURPLE"Func: [%s] "YELLOW"add_after_parse: [%s]\n"RESET
        , node->flag_char, node->flag_val, node->full_name, value_type_to_str(node->value_type), node->min_val, node->max_val, node->multiple_val == VALUE_APPEND ? "append" : node->multiple_val == VALUE_OVERRID ? "override" : "No override", node->nb_stored_val, node->parse ? "Yes" : "No", node->add_value_after_parse ? "Yes" : "No");
...
void free_optvalue(OptNode *opt, U_OptValue *val) {
	...
	if ((opt->value_type > DECIMAL_VALUE)) {
        // ft_printf_fd(1, RED"Freeing opt value: %s\n"RESET, val->str);
		free(val->str);
	}
}
...
void free_opt_node(void *content)
{
    ...
    // ft_printf_fd(1, YELLOW"Freeing opt node: %s: \n"RESET, opt->full_name);
    if (opt) {
        if (opt->full_name) {
            free(opt->full_name);
        }
